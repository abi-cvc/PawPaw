...
public class AdjustUserSlotsServlet extends HttpServlet {
    private static final Logger logger = LoggerFactory.getLogger(AdjustUserSlotsServlet.class);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (updated) {
                ...
                logger.info("Admin {} ajusto slots de usuario {}: {} -> {}", adminId, userId, currentLimit, newLimit);
            } else {
            ...
            logger.error("Error en formato de numeros: {}", e.getMessage());
        } catch (Exception e) {
            session.setAttribute("errorMessage", "Error al procesar la solicitud");
            logger.error("Error al ajustar slots: {}", e.getMessage(), e);
        }
    ...
    private void sendNotificationEmail(User user, int oldLimit, int newLimit, String reason) {
        try {
            ...
            logger.info("Email de notificacion enviado a {}", user.getEmail());

        } catch (Exception e) {
            logger.error("Error al enviar email de notificacion: {}", e.getMessage());
        }
    }
}