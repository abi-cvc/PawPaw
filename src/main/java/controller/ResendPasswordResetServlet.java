...
public class ResendPasswordResetServlet extends HttpServlet {
    private static final Logger logger = LoggerFactory.getLogger(ResendPasswordResetServlet.class);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("Reenvio de token solicitado para: {}", email);
        ...
        if (user != null && user.getActive()) {
            try {
                ...
                if (tokenDAO.create(resetToken)) {
                    ...
                    if (emailSent) {
                        logger.info("Email reenviado a: {}", email);

                        // Actualizar timestamp de último reenvío
                        session.setAttribute("lastResendTime_" + email, currentTime);
                    } else {
                        logger.error("Error al reenviar email");
                    }
                }

            } catch (Exception e) {
                logger.error("Error al reenviar token: {}", e.getMessage(), e);
            }
        } else {
