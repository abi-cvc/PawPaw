        ...
        <main class="main-content">
            ...
            <div class="messages-stats">
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <h3>Total</h3>
                        <p><%= messages != null ? messages.size() : 0 %></p>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <h3>Sin leer</h3>
                        <p><%= unreadCount != null ? unreadCount : 0 %></p>
                    </div>
                </div>
            </div>
            ...
            <div class="filter-section">
                ...
                <div class="filter-buttons">
                    <a href="${pageContext.request.contextPath}/user/messages"
                       class="filter-btn <%= currentFilter == null || currentFilter.isEmpty() ? "active" : "" %>">
                        Todos
                    </a>
                    <a href="${pageContext.request.contextPath}/user/messages?filter=unread"
                       class="filter-btn <%= "unread".equals(currentFilter) ? "active" : "" %>">
                        Sin leer
                        <% if (unreadCount != null && unreadCount > 0) { %>
                        <span class="badge-small"><%= unreadCount %></span>
                        <% } %>
                    </a>
                </div>
            </div>
            ...
            <div class="messages-container">
                <% if (messages != null && !messages.isEmpty()) {
                    for (PetContactMessage msg : messages) {
                        pageContext.setAttribute("msgItem", msg);
                    %>
                <div class="message-card <%= msg.isUnread() ? "unread" : "" %>">
                    <div class="message-header">
                        <div class="message-pet-info">
                            <% if (msg.getPetPhoto() != null && !msg.getPetPhoto().isEmpty()) { %>
                            <img src="<c:out value="${msgItem.petPhoto}"/>" alt="<c:out value="${msgItem.petName}"/>" class="message-pet-photo">
                            <% } else { %>
                            <div class="message-pet-photo-placeholder">🐾</div>
                            <% } %>
                            <div>
                                <h3>Sobre <c:out value="${msgItem.petName}"/></h3>
                                <% if (msg.getPetBreed() != null) { %>
                                <span class="message-pet-breed"><c:out value="${msgItem.petBreed}"/></span>
                                <% } %>
                            </div>
                        </div>
                    ...
                    <div class="message-body">
                        <div class="message-sender">
                            <strong>De:</strong> <c:out value="${msgItem.senderName}"/>
                        </div>
                        <div class="message-phone">
                            <strong>Teléfono:</strong>
                            <a href="tel:<c:out value="${msgItem.senderPhone}"/>"><c:out value="${msgItem.senderPhone}"/></a>
                        </div>
                        <div class="message-time">
                            <c:out value="${msgItem.timeAgo}"/>
                        </div>
                        <div class="message-text">
                            <c:out value="${msgItem.message}"/>
                        </div>
                    </div>

                    <div class="message-actions">
                        <a href="tel:<c:out value="${msgItem.senderPhone}"/>" class="btn btn-primario btn-small">
                            📞 Llamar
                        </a>
                        ...
                        <form method="post" style="display:inline;">
                            ...
                            <input type="hidden" name="messageId" value="<%= msg.getIdMessage() %>">
                            <button type="submit" class="btn btn-secundario btn-small">
                                ✓ Marcar leído
                            </button>
                        </form>
                        <% } %>
