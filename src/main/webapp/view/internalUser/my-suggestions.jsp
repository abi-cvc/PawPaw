...
<body>
    <div class="dashboard">
        ...
        <div class="main-content">
            <div class="topbar">
                <div class="topbar-title"><h1>Mis Sugerencias</h1></div>
                <div class="topbar-actions">
                    <a href="${pageContext.request.contextPath}/user/send-suggestion" class="btn btn-primario">+ Nueva Sugerencia</a>
                </div>
            </div>
            ...
            <div class="content">
                <div class="stats-grid" style="margin-bottom: 2rem;">
                    <div class="stat-card">
                        ...
                        <div class="stat-info"><h3>Total Enviadas</h3><p><c:out value="${totalSuggestions != null ? totalSuggestions : 0}"/></p></div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon" style="background: linear-gradient(135deg, #ffd700 0%, #ff8c00 100%);">⏳</div>
                        <div class="stat-info"><h3>Pendientes</h3><p><c:out value="${pendingCount != null ? pendingCount : 0}"/></p></div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">👁️</div>
                        <div class="stat-info"><h3>Revisadas</h3><p><c:out value="${reviewedCount != null ? reviewedCount : 0}"/></p></div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon" style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);">✅</div>
                        <div class="stat-info"><h3>Resueltas</h3><p><c:out value="${resolvedCount != null ? resolvedCount : 0}"/></p></div>
                    </div>
                </div>
                    ...
                    <c:forEach var="suggestion" items="${suggestions}">
                        <div class="suggestion-card">
                            <div class="suggestion-header">
                                <div>
                                    <span class="status-badge ${suggestion.statusClass}">
                                        <c:out value="${suggestion.statusText}"/>
                                    </span>
                                </div>
                                <span class="suggestion-date">
                                    <c:if test="${suggestion.submissionDate != null}">
                                    <fmt:formatDate value="${suggestion.submissionDate}" pattern="dd/MM/yyyy HH:mm"/>
                                    </c:if>
                                </span>
                            </div>

                            <div class="suggestion-message"><c:out value="${suggestion.message}"/></div>
                            ...
                            <c:if test="${not empty suggestion.adminResponse}">
                                <div class="admin-response" style="margin-top: 1rem;">
                                    ...
                                    <p style="margin: 0.75rem 0 0 0; color: #333; line-height: 1.6;"><c:out value="${suggestion.adminResponse}"/></p>
                                    <c:if test="${suggestion.responseDate != null}">
                                        <div style="font-size: 0.85rem; color: #999; margin-top: 0.5rem;">
                                            Respondido el <fmt:formatDate value="${suggestion.responseDate}" pattern="dd/MM/yyyy HH:mm"/>
                                        </div>
                                    </c:if>
                                </div>
                            </c:if>
                            <c:if test="${empty suggestion.adminResponse && (suggestion.statusSuggestion == 'reviewed' || suggestion.statusSuggestion == 'resolved')}">
                                <div style="margin-top: 1rem; padding: 0.75rem; background: #f0f9ff; border-left: 3px solid var(--color-info); border-radius: var(--radio-sm);">
                                    <p style="margin: 0; color: #666; font-size: 0.9rem;">
                                        ℹ️ Tu sugerencia está siendo ${suggestion.statusSuggestion == 'reviewed' ? 'revisada' : 'procesada'}. Pronto recibirás una respuesta.
                                    </p>
                                </div>
                            </c:if>
                        </div>
                ...
                <c:otherwise>
                    <div class="empty-state">
                        ...
                        <p style="color: #999; margin: 0 0 1.5rem 0;">¿Tienes alguna idea para mejorar PawPaw? ¡Nos encantaría escucharte!</p>
                        <a href="${pageContext.request.contextPath}/user/send-suggestion" class="btn btn-primario">Enviar mi primera sugerencia</a>
                    </div>
                </c:otherwise>
                </c:choose>
            </div>
